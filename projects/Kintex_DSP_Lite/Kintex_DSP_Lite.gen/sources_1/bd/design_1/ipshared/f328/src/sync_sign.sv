		...
		begin		
			...
			begin
					...
					if	(shiftreg_I_pss[shiftreg_number]	==	shiftreg_I_dta[shiftreg_number])	
						sign_reg_I[shiftreg_number]	<=	2'b01 & PSS_mask[pDAT_Num - shiftreg_number];
					else
						sign_reg_I[shiftreg_number]	<=	2'b11 & PSS_mask[pDAT_Num - shiftreg_number];
						
				    if	(shiftreg_Q_pss[shiftreg_number]	==	shiftreg_Q_dta[shiftreg_number])	
						sign_reg_Q[shiftreg_number]	<=	2'b01 & PSS_mask[pDAT_Num - shiftreg_number];
                    else
					    sign_reg_Q[shiftreg_number]	<=	2'b11 & PSS_mask[pDAT_Num - shiftreg_number];
			end		
		end	
